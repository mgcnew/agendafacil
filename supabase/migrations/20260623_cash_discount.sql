-- ─────────────────────────────────────────────────────────────────────────
-- Fase 2 do caixa: desconto na cobrança de cliente.
-- Config por salão + permissão por profissional + desconto no finalize.
-- Regra: "salão absorve" — a comissão continua sobre o preço cheio; só a
-- entrada no caixa cai para (total − desconto).
-- ─────────────────────────────────────────────────────────────────────────

-- 1) Config do salão
alter table salons
  add column if not exists cash_discount_enabled boolean not null default false,
  add column if not exists cash_max_discount_percent numeric not null default 0;

-- 2) Permissão (aparece na aba Permissões da Equipe; owner já tem tudo)
insert into permissions (category, key, label)
values ('Caixa', 'cash.discount', 'Dar desconto no caixa')
on conflict (key) do nothing;

-- 3) finalize_appointment com desconto opcional (default 0 = comportamento atual)
...
begin
  ...
  if v_session is not null and v_net > 0
    ...
    values
      (v_salon, v_session, 'income', 'atendimento', v_net, p_payment_method, p_appointment, v_member,
       'Atendimento' || coalesce(' · ' || v_client, '') || (case when v_disc > 0 then ' (desc. ' || trim(to_char(v_disc, 'FM999990.00')) || ')' else '' end),
       auth.uid());
