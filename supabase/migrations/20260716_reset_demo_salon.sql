-- Semeador/reset de salão demo. Limpa e repovoa serviços, clientes e uma
-- semana de agenda (passado concluído + próximos dias) com caixa e comissão,
-- por vertical (feminino/barbearia). Datas relativas a "hoje" pra a agenda
-- parecer sempre atual. Usado pelo reset agendado (pg_cron) e sob demanda.
-- Só roda em salões is_demo (guarda de segurança).
...
begin
  ...
  select id into v_member from salon_members where salon_id = p_salon and role = 'owner' and is_active limit 1;
...
revoke all on function public.reset_demo_salon(uuid) from anon, authenticated;
