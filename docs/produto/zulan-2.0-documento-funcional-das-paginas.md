# ZULAN 2.0

# Documento Funcional das Páginas

> Este documento descreve cada módulo do sistema, sua função atual, a evolução proposta e como a Inteligência Artificial transforma cada tela em uma ferramenta de gestão.

---

# Dashboard

## Objetivo

Ser o centro operacional do salão.

O empreendedor deve abrir o sistema e, em menos de um minuto, entender exatamente como está seu negócio e quais ações precisam ser tomadas.

## Situação Atual

O Dashboard apresenta:

- Agendamentos do dia
- Próximos clientes
- Faturamento
- Indicadores rápidos

Funciona como um resumo operacional.

## Evolução Proposta

O Dashboard passa a ser comandado pelo Gestor Inteligente.

Ao acessar o sistema o usuário encontra uma equipe virtual já trabalhando.

Exemplo:

"Bom dia, Marcelo.

Sua equipe encontrou 6 oportunidades hoje.

✓ 3 clientes podem voltar.

✓ Seu estoque está baixo.

✓ Amanhã existem horários vagos.

✓ Hoje é aniversário de duas clientes.

✓ Seu ticket médio caiu.

✓ Existe oportunidade de vender kits de hidratação."

## Papel da IA

- Priorizar tarefas
- Resumir indicadores
- Mostrar oportunidades
- Alertar riscos
- Sugerir ações
- Executar ações rápidas

## Benefício

O Dashboard deixa de mostrar números.

Ele mostra prioridades.

---

# Agenda

## Objetivo

Organizar o fluxo de atendimento do salão.

## Situação Atual

- Agenda por dia
- Semana
- Mês
- Novo agendamento
- Bloqueio de horário

## Evolução

A agenda deixa de ser apenas um calendário.

Ela passa a ser inteligente.

A IA identifica:

- Horários vazios
- Cancelamentos
- Clientes atrasados
- Horários mais lucrativos
- Oportunidades de encaixe
- Dias fracos

## Ações Inteligentes

A IA poderá:

- sugerir encaixes
- remarcar clientes
- preencher horários vagos
- enviar confirmações
- enviar lembretes
- abrir horários extras
- sugerir campanhas para dias frios

## Benefício

Agenda cheia.

Menos cancelamentos.

Maior ocupação.

---

# Clientes

## Objetivo

Centralizar todo o relacionamento.

## Evolução

Cada cliente deixa de ser apenas um cadastro.

Passa a possuir um histórico inteligente.

## A IA conhece

- frequência
- ticket médio
- aniversários
- produtos preferidos
- profissional favorito
- cancelamentos
- faltas
- serviços realizados
- potencial de compra

## A IA poderá

- prever abandono
- sugerir campanhas
- criar relacionamento
- lembrar retornos
- sugerir produtos
- identificar clientes VIP

---

# Recuperação de Clientes

## Objetivo

Trazer clientes de volta automaticamente.

## Situação Atual

Campanhas simples.

## Evolução

A IA identifica automaticamente:

- clientes perdidos
- clientes inativos
- clientes que costumavam vir
- clientes cancelados
- clientes com retorno atrasado

## Ações

Criar campanhas.

Enviar mensagens.

Agendar retorno.

Mensurar resultados.

Aprender quais campanhas funcionam melhor.

## Objetivo

Nunca deixar dinheiro parado.

---

# Serviços

## Objetivo

Garantir que cada serviço do cardápio realmente valha a pena pro salão, não só pro cliente.

## Situação Atual

- Cadastro (nome, preço, duração, comissão, categoria, produtos consumidos)
- Margem estimada na hora de configurar — nunca confrontada com o que realmente aconteceu

## Evolução

O catálogo deixa de ser só configuração e passa a refletir a realidade: quanto cada serviço realmente trouxe, não só quanto deveria trazer.

## A IA conhece

- quantas vezes cada serviço foi agendado
- receita real gerada (não só preço de tabela)
- margem real (preço − comissão − produto consumido, comparada com a estimada)
- serviços parados (ninguém agenda há X dias)
- relação tempo ocupado × retorno (vale a pena segurar a agenda 2h nesse serviço?)
- serviço mais pedido por profissional

## Ações Inteligentes

A IA poderá:

- alertar serviço parado (mesmo princípio do "cliente inativo")
- avisar quando a margem real ficou negativa ou abaixo do esperado — hoje isso é invisível
- apontar serviços que ocupam muita agenda e rendem pouco
- sugerir reajuste de preço com base no padrão de procura

## Benefício

Cardápio que se paga sozinho — menos serviço no escuro, mais decisão com base no que realmente aconteceu.

---

# Pacotes

## Objetivo

Ajudar o dono a precificar pacotes com informação, não no escuro — tanto na criação quanto no acompanhamento dos já vendidos.

## Situação Atual

Modelos reutilizáveis (nome, serviços incluídos, validade, preço digitado manualmente). Pacotes vendidos mostram sessões restantes e vencimento. Zero estatística agregada.

## Evolução

Na hora de criar o modelo, o dono vê o impacto real da decisão, não só um campo de preço vazio. Nos pacotes já vendidos, a IA aponta o que precisa de atenção.

## A IA conhece

- preço avulso somado dos serviços incluídos vs. preço do pacote → desconto efetivo real
- margem real desses serviços (reaproveitando o que já existe em Serviços) → margem por sessão dentro do pacote
- pacotes vencendo
- pacotes comprados e nunca usados (sinal de risco/insatisfação)
- modelo mais vendido / que mais gera receita

## Ações Inteligentes

A IA poderá:

- ao criar o modelo: mostrar desconto efetivo e margem estimada *antes* de salvar (cálculo direto, sem IA generativa — é conta, não opinião)
- alertar pacote parado (comprado, zero sessão usada) com sugestão de lembrete de 1 clique
- trazer "vencendo" também pra essa página, não só no Dashboard

## Benefício

Pacote vendido com discernimento, não no escuro — e nenhum cliente que pagou antecipado é esquecido.

---

# Estoque

## Objetivo

Controlar insumos e produtos vendidos.

## Situação Atual

Cadastro único para:

- insumos
- produtos para venda

## Evolução

O estoque passa a ser preditivo.

A IA acompanha:

- consumo
- desperdício
- giro
- margem
- produtos parados
- previsão de reposição

## A IA poderá

Alertar reposição.

Sugerir compras.

Identificar desperdícios.

Sugerir promoções.

Cruzar vendas com serviços.

Calcular lucro real.

## Benefício

Menos desperdício.

Maior lucro.

---

# Inteligência do Negócio

*(Antiga página Financeiro)*

## Objetivo

Transformar números em decisões.

## A IA interpreta

Não mostra apenas gráficos.

Ela responde:

Por que faturou menos?

Por que vendeu mais?

Qual dia está ruim?

Qual profissional cresce?

Onde existe desperdício?

Como aumentar lucro?

## Abas

Financeiro

Inteligência

Temperatura

Crescimento

Assistente IA

## Benefício

O empresário recebe decisões.

Não gráficos.

---

# Equipe

## Objetivo

Gerenciar pessoas.

## Evolução

Cada colaborador possui um painel inteligente.

## Indicadores

Clientes atendidos.

Ticket médio.

Retenção.

Produtos vendidos.

Comissão.

Cancelamentos.

Agenda futura.

Produtividade.

## IA

Detecta oportunidades.

Sugere treinamentos.

Mostra evolução.

Cria metas.

Reconhece destaques.

Ajuda distribuir agenda.

## Benefício

Transformar gestão de equipe em crescimento contínuo.

---

# Campanhas

## Objetivo

Atrair clientes continuamente.

## Evolução

Campanhas deixam de ser criadas manualmente.

A IA identifica oportunidades automaticamente.

## Tipos

Clientes inativos

Aniversário

Retorno

Dias frios

Produtos

Promoções

Clientes VIP

Primeira visita

Indicação

## Papel da IA

Escolher público.

Escolher horário.

Criar texto.

Enviar.

Analisar resultados.

Aprender continuamente.

## Benefício

Marketing funcionando sozinho.

---

# Princípio Geral

Nenhuma página do Zulan existe apenas para armazenar dados.

Toda página deve responder:

Como esta tela aumenta o faturamento do salão?

Como esta tela economiza tempo?

Como esta tela melhora a experiência do cliente?

Se uma página não responder a essas perguntas, ela deverá ser redesenhada.
